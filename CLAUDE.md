...
- **Seção Férias Equipe totalmente funcional (sessão 23)** — corrigidos 4 bugs e aplicado cache localStorage:
  ...
  - Bug de dados sumindo no F5 corrigido: `mostrarSiteAutenticado()` não chamava `loadEquipeFirestore()` quando os JSONs já tinham carregado antes da autenticação. Commit `ddb11a9`.
  - Bug de calendário vazio corrigido: `loadEquipeFirestore()` agora re-renderiza o calendário ao terminar se a seção equipe estiver ativa. Commit `054fef2`.
  - Cache localStorage `pma-equipe-fs` adicionado: dados exibidos instantaneamente no F5, sem delay. Lógica extraída em `_equipeAplicarRegistros(registros)`. Commit `261278d`.
- **Badges da Férias Equipe por pessoa com cor única (sessão 23)** — removida legenda de tipo (férias/folga/outro) do rodapé. Cada pessoa recebe cor única e estável da paleta `EQUIPE_PESSOA_PALETTE`, atribuída em ordem alfabética dos nomes via `_equipeCorPessoa(nome)`. Cores fixas em `EQUIPE_PESSOA_CORES_FIXAS` têm prioridade sobre a paleta. Cores atuais fixadas: Fábio=ciano `#06b6d4`, Larice=rosa pink `#ec4899`, Natália=teal `#14b8a6`, Luma=amarelo vivo `#eab308`. Commits `f4dc21c`, `8066d0e`, `62b2cee`, `63a3669`, `00066e7`.
...
- **Tabela Adote — 5 linhas × 4 colunas** — colunas: Defensor(a) Público(a) (26%), Comarcas (18%), Servidor(a) (22%), E-mail (34%). Coluna "Forma de Trabalho" **removida na sessão 19**. Linha do Raimundo Sérvulo Lourido Barreto (Tapauá) **removida na sessão 19**. Defensores atuais: Carlos Alberto (SSU), Wilson Oliveira (Silves), Antonio Cavalcante (Itapiranga), Ulysses Falcão (Urucurituba), Thiago Nobre (Urucará). Dados estáticos em `ADOTE_STATIC[]` (5 objetos) e `ADOTE_COLS`/`ADOTE_COL_LABELS`. Células editáveis com `data-adote-key="ROW_COL"`. Mesmo mecanismo de dupla aplicação das Atribuições. Persistidas em `secoes/adote_celulas`. Cache em memória `_adoteCelulasCache`. Globals análogos: `_adoteModoEdicao`, `_adoteCelulaAtiva`, `_adoteCelulasBackup`.
- **Cabeçalho editável do Adote** — `<h2>` com `<span id="adote-cabecalho-texto">` e botão "✏️ Editar info" (sessão 19: migrado de `span.adote-cabecalho-txt` para `<h2>`, uniformizando com o título do bloco Expandir). Form inline `#adote-info-form` com campos: Cabeçalho, Nome da portaria, Link do PDF. Persistido em `secoes/adote_info`. Padrão: "14º Ciclo do Projeto Adote uma Comarca · Vigência: 16/03 a 15/06/2026". **Atenção CSS:** botões dentro do form usam regra `#adote-info-form .btn-salvar, .btn-cancelar { display: inline-flex }` para sobrepor o `display:none` global.
...
- **Cache localStorage da seção Adote (sessão 19)** — `pma-adote-celulas` e `pma-adote-expandir` em `localStorage`. `_adoteCarregarCelulas()` e `_adoteCarregarExpandir()` aplicam o cache localStorage instantaneamente (síncrono) antes de buscar do Firestore, eliminando flash de dados estáticos no F5. Firestore ainda é consultado a cada abertura para manter o cache fresco. `_adoteSalvarCelulas()` e `salvarSecao('adote_expandir')` atualizam o localStorage ao salvar. **Detecção de cache desatualizado:** `_adoteCacheDesatualizado(celulas)` verifica se `maxRow > ADOTE_STATIC.length`; se verdadeiro, descarta localStorage + Firestore e (admin) deleta `secoes/adote_celulas` automaticamente. Evita exibição de linhas removidas do `ADOTE_STATIC`.
...
- **Nova seção "Férias Equipe" (sessão 20, melhorada na sessão 23)** — botão ⛱️ azul-céu (`#0369a1 → #38bdf8`) na landing page e no header. Seção `#equipe` sem barra de abas. Calendário de badges por mês com seletor de ano (2026/2027) e seletor de meses. Todos os dias são clicáveis (antes apenas dias com afastamentos). Modal de detalhes ao clicar no dia (tabela com nome, tipo, período; botões ✏️ 🗑️ para admins). Formulário CRUD: Nome (texto livre), Data início, Data fim, Tipo (Férias/Folga/Outro). Coleção Firestore: `afastamentos_equipe`. Globals: `equipeCurrentYear`, `equipeCurrentMonth`, `equipeAfastamentos[ano][mes][dia]`, `equipeMap`. Funções: `_equipeAplicarRegistros(registros)`, `loadEquipeFirestore()`, `switchEquipeYear(year)`, `renderEquipeCalendar()`, `abrirModalEquipe(ano,mes,dia)`, `fecharModalEquipe()`, `abrirFormEquipe(id,ano,mes,dia)`, `fecharFormEquipe()`, `salvarEquipeFirestore()`, `confirmarDeletarEquipe(id,ano,mes,dia)`, `_fmtEquipeDate(str)`. **Cores por pessoa:** `_equipeCorPessoa(nome)` usa `EQUIPE_PESSOA_CORES_FIXAS` (prioridade) ou `EQUIPE_PESSOA_PALETTE` (dinâmica, atribuída em ordem alfabética). Cores fixas: Fábio=`#06b6d4`, Larice=`#ec4899`, Natália=`#14b8a6`, Luma=`#eab308`. Legenda de tipo removida do rodapé. Cache `pma-equipe-fs` no localStorage elimina delay no F5.
...
| Férias Equipe (docs Firestore brutos) | `pma-equipe-fs` | `afastamentos_equipe` | `loadEquipeFirestore()` via `_equipeAplicarRegistros()` | `loadEquipeFirestore()` após salvar/deletar |
